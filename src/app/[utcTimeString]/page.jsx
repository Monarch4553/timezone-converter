...
export default function ViewRequestedTime({ params }) {
  ...
  // Local state to store the user's timezone and the converted time
  const [localTimeFormatted, setLocalTimeFormatted] = useState(null);
  ...
  useEffect(() => {
    ...
    const localTime = moment.utc(decodedUtcTimeString).tz(timezone);
    setLocalTimeFormatted(localTime); // Set the local time in the state
  }, [decodedUtcTimeString]);

  if (!localTimeFormatted) {
    return <div>Loading...</div>; // Show loading state until the time is set
  }
  ...
  return (
    <div className="gradient-bg flex items-center h-screen justify-center">
      <Head>
        ...
        <title>Local Time: {localTimeFormatted.format("HH:mm")}</title>
        <meta
          name="description"
